...
namespace ArgentSea.Pg.Test
{
    public class ConfigurationTests
    {
        ...
        public void TestConfigurationOptions()
        {
            ...
            pgDbData.PgDbConnections[0].ReadConnection.GetConnectionString(null).Should().Be("Application Name=MyApp;Multiplexing=True;Database=MainDb;Host=10.10.25.1", "this is the value inherited from global configuration settings.");
            pgDbData.PgDbConnections[0].WriteConnection.GetConnectionString(null).Should().Be("Application Name=MyApp;Multiplexing=True;Database=MainDb;Host=10.10.25.1", "this is the value inherited from global configuration settings.");
            pgDbData.PgDbConnections[1].ReadConnection.GetConnectionString(null).Should().Be("Application Name=MyOtherApp;Multiplexing=True;Database=OtherDb;Host=10.10.25.2", "this is the value that overrides the global setting");
            pgDbData.PgDbConnections[1].WriteConnection.GetConnectionString(null).Should().Be("Application Name=MyOtherApp;Multiplexing=True;Database=OtherDb;Host=10.10.25.2", "this is the value that overrides the global setting");
        ...
        public void TestServiceBuilder()
        {
            ...
            dbService["MainDb"].Read.ConnectionString.Should().Be("Application Name=MyApp;Multiplexing=True;Database=MainDb;Host=10.10.25.1", "this is the value inherited from global configuratoin settings.");
            dbService["OtherDb"].Read.ConnectionString.Should().Be("Application Name=MyOtherApp;Multiplexing=True;Database=OtherDb;Host=10.10.25.2", "this is the value that overrides the global setting");
            ...
            shardService["Inherit"][0].Read.ConnectionString.Should().Be("Application Name=Inheritance;Multiplexing=True;Host=10.10.25.3", "the configuration file builds this connection string");
            shardService["Inherit"][0].Write.ConnectionString.Should().Be("Application Name=Inheritance;Multiplexing=True;Host=10.10.25.3", "the configuration file builds this connection string");
            shardService["Inherit"][1].Read.ConnectionString.Should().Be("Application Name=Inheritance;Multiplexing=True;Host=10.10.25.4", "the configuration file builds this connection string");
            shardService["Inherit"][1].Write.ConnectionString.Should().Be("Application Name=Inheritance;Multiplexing=True;Host=10.10.1.5", "the configuration file builds this connection string");
            shardService["Explicit"][0].Read.ConnectionString.Should().Be("Application Name=MyWebApp;Multiplexing=True;Password=pwd2345;Username=user;Integrated Security=True;Auto Prepare Min Usages=6;Cancellation Timeout=22;Check Certificate Revocation=True;Client Certificate=xyz;Client Certificate Key=abc;Client Encoding=UTF16;Command Timeout=16;Connection Idle Lifetime=301;Connection Lifetime=30;Connection Pruning Interval=11;Convert Infinity DateTime=True;Database=MyDb1;Encoding=UTF16;Enlist=False;Host=10.10.25.6;Include Error Detail=False;Include Realm=False;Internal Command Timeout=100;Keepalive=100;Kerberos Service Name=test;Load Table Composites=True;Log Parameters=False;Max Auto Prepare=1;Maximum Pool Size=99;Minimum Pool Size=2;No Reset On Close=True;Options=123;Passfile=c:\\passfile.pwd;Persist Security Info=False;Pooling=False;Port=5433;Read Buffer Size=4096;Root Certificate=cert1;Search Path=;Server Compatibility Mode=None;Socket Receive Buffer Size=4096;Socket Send Buffer Size=4096;SSL Mode=Disable;TCP Keepalive=False;Timeout=16;Timezone=America/Chicago;Trust Server Certificate=False;Write Buffer Size=4096;Write Coalescing Buffer Threshold Bytes=2000;Write Coalescing Delay Us=1", "the configuration file builds this connection string");
